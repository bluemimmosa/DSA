...
void LinkedList::reverse(){
	//lets check if the list is empty
	Node *cur = head;
	Node *tmp;
	if(cur == NULL){
	...
	while(cur != NULL){
		tmp = cur->prev; //saving prev so it doesnt get lost.
		cur->prev = cur->next; // updating prev with new value.
		cur->next = tmp; //updating next with new value.
		cur = cur->prev; //always make sure you use the updated pointes.
						// so dont use cur = cur->next as cur->next will have null
...
int main(){
	...
	do{
		...
		cout<<"6. Display all nodes from tail to head."<<endl;
